include("gasfun_fine.jl")
include("gascalc_fine.jl")
include("tfan_fine.jl")
include("tfmap_fine.jl")
include("tfcool_fine.jl")
include("engine_data_structure.jl")
...
function tfsize(engine_variable_obj::engine.engine_variable)
    ...
    # operation_design_variable_obj::operation_design_variable


    # Continuous design var
    ...
    ifuel = engine_variable_obj.design_variable_integer_obj.ifuel # (molecular composition / h / rho)
    ...
    for ipass = 1:npass
        ...
        if (ipass >= 2)
            ...
            if (abs(dmfrac) < toler)
                ...
                state_5_s = engine.station_state_static(u5, M5, T5, nothing, p5, cp5, R5, A5)
                state_6_s = engine.station_state_static(u6, M6, T6, nothing, p6, cp6, R6, A6)
                state_7_s = engine.station_state_static(u7, M7, T7, nothing, p7, cp7, R7, A7)
                ...
                return on_design_output_full_engine_obj
            end
